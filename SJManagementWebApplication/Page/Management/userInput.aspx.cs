...
namespace SJManagementWebApplication
{
    public partial class userInput : System.Web.UI.Page
    {
        ...
        //현재 페이지 파일 경로 설정
        string sPagePath = "Management/userInput";
        ...
        protected void RowDeleteOk(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //삭제 쿼리
                sbQuery.Append("DELETE FROM US001 ");
                sbQuery.Append("WHERE us_seq = @seq");
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            sInventor = tbphoneTxt.Text;
            sNum = tbidLab.Text;
            sAppnum = cbjobTxt.Text;
            sFill = cbdepartTxt.Text;
            ...
            else
            {
                ...
                switch (btnNew.ImageUrl.ToString().Equals(NumDefine.ADD_IMAGE))//신규버튼 활성화 상태로 판단
                {
                    case true:
                        ...
                        {
                            //업데이트 실행
                            sbQuery.Append("UPDATE US001 SET us_name = @name,us_phone = @phone,us_id = @id,us_job = @job, us_depart = @depart, ");
                    ...
                    case false:
                        ...
                        try
                        {
                            ...
                            //추가 쿼리
                            sbQuery.Append("INSERT INTO US001 ");
                            sbQuery.Append("VALUES((select isnull(max(us_seq),0) + 1 from US001),");
        ...
        protected void TextChanged(object sender, EventArgs e)
        {
            ...
            if (btnNew.ImageUrl.ToString().Equals(NumDefine.CANCEL_IMAGE)) //신규 등록 상태
            {
                ...
                {
                    sbQuery.Append("SELECT us_phone, us_id FROM US001 ");
                    sbQuery.Append("WHERE us_name = @sText");

                    dCondition.Add("@sText", tbNameTxt.Text);

                    ShowOverlayPopup(OverlayCheck.CheckOverlay(sbQuery, dCondition, 2), "us_name");
                }
                else if (sender.Equals(tbNameTxt))
                {
                    sbQuery.Append("SELECT us_name, us_phone FROM US001 ");
                    sbQuery.Append("WHERE us_id = @sText");

                    dCondition.Add("@sText", tbNameTxt.Text);

                    ShowOverlayPopup(OverlayCheck.CheckOverlay(sbQuery, dCondition, 2), "us_id");
                }
                else if (sender.Equals(tbidLab))
                {
                    sbQuery.Append("SELECT us_name, us_phone FROM US001 ");
                    sbQuery.Append("WHERE us_phone = @sText");

                    dCondition.Add("@sText", tbidLab.Text);

                    ShowOverlayPopup(OverlayCheck.CheckOverlay(sbQuery, dCondition, 2), "us_phone");
                }
            }
        }
        //중복 시 팝업 3종
        protected void ShowOverlayPopup(string[] sOverlayData, string sType)
        {
            ...
            try
            {
                if (int.Parse(sOverlayData[0]) > 0)
                {
                    ...
                    if (sType.Equals("us_name"))
                        pcPopup.showPopup(true, true, "다음의 항목이 중복 검색되었습니다. 계속 진행하시겠습니까?\n\n[직원명] " + tbNameTxt.Text + "\n[연락처] " + sOverlayData[1] + "\n[ID] " + sOverlayData[2]);
                    else if (sType.Equals("us_phone"))
                        pcPopup.showPopup(true, true, "다음의 항목이 중복 검색되었습니다. 계속 진행하시겠습니까?\n\n[직원명] " + sOverlayData[1] + "\n[연락처] " + tbidLab.Text + "\n[ID] " + sOverlayData[2]);
                    else if (sType.Equals("us_id"))
                        pcPopup.showPopup(true, true, "다음의 항목이 중복 검색되었습니다. 계속 진행하시겠습니까?\n\n[직원명] " + sOverlayData[1] + "\n[연락처] " + sOverlayData[2] + "\n[ID] " + tbNameTxt.Text);
                    //파일리스트 새로고침
                    tbFileslabel.Focus();
                }
        ...
        protected void BtnViewfile_Click(object sender, EventArgs e)
        {
            if (gdPatentManageInput.FocusedRowIndex >= 0 && gdPatentManageInput.VisibleRowCount != 0)
            {
                ...
                DirectoryInfo diFolder = new DirectoryInfo(@"" + Server.MapPath("~/") + sFilePath + gdPatentManageInput.GetRowValues(gdPatentManageInput.FocusedRowIndex, "us_seq").ToString()); //파일 폴더 경로
                //만약 폴더가 존재하지 않으면 생성
        ...
        protected void Upload()
        {
            ...
            {
                sbQuery.Append("SELECT ISNULL(MAX(us_seq),0)+1 ");
                sbQuery.Append("FROM US001");
